...
public class Books extends Database
{
	...
	{
		Database.viewIssuedBook();
	}
	
	public static void addBook(String name)
	{
		Books book = new Books(name);
		Database.addBookData(name, book);
	}
	
	public static void deleteBook(String name)
	{
		HashMap<String, Books> book = Database.getAllBookData();
		book.remove(name);
		HashSet<String> name1 = Database.getAllBookNames();
		name1.remove(name);
		Database.deleteBookData(name);
	}
	...
	public static Books getBook(String name)
	{
		...
		return Database.getBookData(name);
	}
	...
	{
		HashSet<String> name = Database.getAllBookNames();
		for(String i : name)
		{
	...
	public void editBookDetails(String name)
	{
		...
		if(n==1)
		{
			...
			book.name=newName;
			Database.updateBookKey(name, newName);
		}
		else if(n==2)
		{
		...
		else if(n==4)
		{
			...
			User user = Database.getUserData(uid);
			book.userId= user;
